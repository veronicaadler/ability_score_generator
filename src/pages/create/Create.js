...
const Create = () => {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      .then(([data1, data2]) => {
        //here we are using the data retrieved from the fetch
      ...
      .catch((err) => {
        if (err.name === "AbortError") {
          ...
          //catches any network errors, like not being able to connect to server
  ...
  if (error || isPending) {
    ...
    return (
      <CharacterForm allClasses={possibleClasses} allRaces={possibleRaces} />
    );
  }
};
